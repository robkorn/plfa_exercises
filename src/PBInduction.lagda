...
\begin{code}
myassoc : ∀ (m n p : ℕ) → (m + n) + p ≡ m + (n + p)
myassoc zero n p = refl
myassoc (suc m) n p rewrite myassoc m n p = refl
...
+-swap m n p rewrite (sym (myassoc m n p)) | (+-comm m n) | myassoc n m p = refl
...
*-distrib-+ (suc m) n p rewrite *-distrib-+ m n p | myassoc p (m * p) (n * p) = refl
...
-- This answer is extremely contrived for this simple of a theorem, but hey it works.
multZeroLemma : ∀ (m : ℕ) -> m * zero ≡ zero
multZeroLemma zero = refl
multZeroLemma (suc m) = multZeroLemma m

*-assoc : ∀ (m n p : ℕ) -> (m * n) * p ≡ m * (n * p)
*-assoc zero n p = refl
*-assoc (suc m) zero zero = *-assoc m zero zero
*-assoc (suc m) (suc n) zero rewrite multZeroLemma (n + m * suc n) | multZeroLemma n | multZeroLemma m = refl
*-assoc (suc m) n (suc p) rewrite (*-assoc m n (suc p)) | (sym (*-assoc m n (suc p))) | *-distrib-+ n (m * n) (suc p) = refl
